...
def plot_graph(dirpath):
    ...
    for obj in gemd_objects:
        ...
        if obj_type.endswith('spec') or obj_type.endswith('template') or obj_type.startswith('parameter') or obj_type.startswith('condition') or obj_type.startswith('property'):
            nb_disregarded +=1
            continue
...
def test(encoder, subdirpath, terminal_gemd_obj):
    '''
